...
The DEV-only `card-gallery` route is reached via the `window.__holdfast.showCardGallery()` dev hook. It is captured on a **fresh page** in the same browser context (`capture_card_gallery`), not off the walked page: the gallery is a standalone showcase with no campaign dependency, and isolating its render from the walk's pending timers/promises is what makes its dark-fantasy baseline deterministic. Before capture, the harness asserts all 21 unique JSON cards, exact five-layer scenes, mixed SVG/PNG art motifs, loaded SVG effect glyphs, tall-card and near-square-art geometry, foreground contrast, selection interaction, the upgraded exemplar, and a vector-only card back whose loaded `arcane_burst.svg` emblem comes from the active Runic vocabulary. It then removes only the gallery's internal scroll constraint and commits one deterministic 1440-wide full-height contact sheet containing the complete catalog, exemplar pair, and back; the other seven baselines remain normal viewport captures. The route is `import.meta.env.DEV`-gated and tree-shaken from production builds.
...
| `walk` / `capture_game_over` / `capture_card_gallery` | Drivers and semantic assertions for screens reached outside the linear advance path. |
